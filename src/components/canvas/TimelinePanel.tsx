...
export function TimelinePanel({ entries, onExport }: { entries: Entry[]; onExport: () => void }) {
            ...
            <div className="space-y-3">
              {groups.get(k)!.map((e) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="bg-background p-3 border-2 border-ink"
                    style={{ borderLeftWidth: 6, borderLeftColor: `var(--${tone})` }}
                  >
                    <div className="pixel text-[8px] opacity-60 mb-2">{time}</div>
